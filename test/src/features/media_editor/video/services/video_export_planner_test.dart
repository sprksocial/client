...
import 'package:spark/src/features/media_editor/video/services/audio_source_resolver.dart';
import 'package:spark/src/features/media_editor/video/services/video_export_planner.dart';
import 'package:spark/src/features/media_editor/video/services/video_timing_export_service.dart';

void main() {
  test('keeps an unedited portrait video in source space', () {
    final transform = buildVideoExportTransform(
      parameters: _parameters(),
      profile: MediaEditorProfile.post,
  ...
  test('crops a landscape story video to the story canvas', () {
    final transform = buildVideoExportTransform(
      parameters: _parameters(),
      profile: MediaEditorProfile.story,
      sourceSize: const Size(1920, 1080),
      storyCanvasSize: const Size(1440, 2560),
    );

    expect(transform, isNotNull);
    expect(transform!.width, 608);
    expect(transform.height, 1080);
    expect(transform.x, 656);
