...
import 'package:spark/src/features/media_editor/video/services/audio_helper_service.dart';
import 'package:spark/src/features/media_editor/video/ui/controllers/video_editor_export_controller.dart';
import 'package:spark/src/features/media_editor/video/ui/controllers/video_editor_media_session.dart';
import 'package:spark/src/features/media_editor/video/ui/controllers/video_editor_preview_asset_loader.dart';
import 'package:spark/src/features/media_editor/video/ui/widgets/audio/audio_range_selection_overlay.dart';
import 'package:spark/src/features/media_editor/video/ui/config/video_editor_configs_builder.dart';
import 'package:spark/src/features/media_editor/video/ui/widgets/common/video_initializing_widget.dart';
import 'package:spark/src/features/media_editor/video/ui/widgets/layout/video_editor_regular_chrome.dart';
import 'package:spark/src/features/media_editor/video/ui/widgets/player/video_player_widget.dart';
...
class VideoEditorPage extends StatefulWidget {
  ...
  static Future<VideoEditorResult?> open(
    ...
    required MediaEditorProfile profile,
  ...
  final MediaEditorProfile profile;
...
class _VideoEditorPageState extends State<VideoEditorPage>
    with StoryMentionEditing<VideoEditorPage>, SingleTickerProviderStateMixin {
  ...
  late final VideoEditorPreviewAssetLoader _previewAssetLoader;
  late final VideoEditorExportController _exportController;
  ...
  void initState() {
    super.initState();
    _previewAssetLoader = VideoEditorPreviewAssetLoader(
      widget.processingService,
    );
    _exportController = VideoEditorExportController(
      taskId: _taskId,
  ...
  Future<void> _initializePlayer() async {
    final metadataFuture = widget.processingService.getMetadata(_video);
    ...
    try {
      ...
      if (widget.profile == MediaEditorProfile.story) {
      ...
      final proVideoController = ProVideoController(
        ...
        initialResolution: widget.profile == MediaEditorProfile.story
            ? _storyCanvasSize
      ...
      audioAudition = AudioAuditionController(
        VideoEditorAudioAuditionPlayback(audioPlayback),
      ...
      if (widget.profile == MediaEditorProfile.post) {
  ...
  void _onAudioAuditionChanged() {
    if (!mounted) return;
    if (_audioAudition?.isActive == true) {
      _shouldResetOnPlaybackComplete = false;
    }
    _regularChrome?.setOverlayActive(
      _audioAudition?.state?.blocksHostInteraction ?? false,
    );
    setState(() {});
  }
  ...
  BoxFit get _videoFit => widget.profile == MediaEditorProfile.story
      ? BoxFit.cover
      : BoxFit.contain;
  ...
  void _onDurationChange() {
    ...
    if (audition != null && audition.isActive) {
      final playbackSnapshot = AudioAuditionPlaybackSnapshot(
        position: videoValue.position,
        isPlaying: videoValue.isPlaying,
        isCompleted: videoValue.isCompleted,
      );
      final rangeState = audition.handlePlaybackSnapshot(playbackSnapshot);
      if (rangeState != null) {
        unawaited(
          _synchronizePlayback(
            rangeState.draft,
            rangeState.playbackSpan,
            playbackSnapshot,
            isCurrent: () =>
                mounted &&
                identical(_audioAudition?.rangeState, rangeState) &&
                !rangeState.isScrubbing,
          ),
        );
      }
      return;
    ...
    if (audioTrack != null) {
      unawaited(
        _synchronizePlayback(
          ...
          isCurrent: () =>
              mounted &&
              !(_audioAudition?.isActive ?? false) &&
              identical(_proVideoController?.audioTrack, audioTrack),
        ),
      );
    }
  ...
  Future<void> _synchronizePlayback(
    ...
    try {
      await _audioPlayback?.synchronize(
        track,
        playbackSpan,
        snapshot,
        isCurrent: isCurrent,
  ...
  void _commitAudioAudition(AudioAuditionResult result) {
    _shouldResetOnPlaybackComplete = _videoController.value.isPlaying;
    _proVideoController?.audioTrack = result.track;
          ...
          ProImageEditor.video(
            ...
            callbacks: ProImageEditorCallbacks(
              ...
              videoEditorCallbacks: VideoEditorCallbacks(
                onPause: () {
                  _shouldResetOnPlaybackComplete = false;
                  if (audition?.isActive == true) return;
                  final playback = _audioPlayback;
                  if (playback != null) {
                    unawaited(playback.pauseEditorPlayback());
                  }
                },
                onPlay: () {
                  final playback = _audioPlayback;
                  if (playback == null ||
                      playback.rejectPlayRequest(
                        auditionActive: audition?.isActive == true,
                      )) {
                    return;
                  }
                  ...
                  final audioTrack = _proVideoController?.audioTrack;
                  unawaited(
                    playback.playEditorPlayback(
                      audioTrack,
                      _playbackSpan,
                      isCurrent: () =>
                          mounted &&
                          identical(_audioPlayback, playback) &&
                          !(_audioAudition?.isActive ?? false) &&
                          identical(
                            _proVideoController?.audioTrack,
                            audioTrack,
                          ),
                    ),
                  );
