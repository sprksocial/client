...
class FeedPageStateNotifier extends _$FeedPageStateNotifier {
  ...
  Future<void> _fetchFeed(int feedType) async {
    state = state.copyWith(isLoading: true, errorMessage: null, currentIndex: 0);
  ...
  List<FeedPost> _removeDuplicatePosts(List<FeedPost> posts) {
    ...
    for (final post in posts) {
      final isDuplicate = uniquePosts.any((uniquePost) => uniquePost.uri == post.uri && uniquePost.cid == post.cid);
  ...
  Future<void> _preloadMedia(int index, FeedPost post) async {
    ...
    ref.read(preloadMediaProvider(index: index, videoUrl: post.videoUrl, imageUrls: post.imageUrls).future);
  }
  ...
  void updateIndex(int newIndex) {
    if (state.currentIndex != newIndex) {
      ...
      _preloadRepository.updateLoadedMedia(newIndex, state.currentIndex, state.posts.length);
