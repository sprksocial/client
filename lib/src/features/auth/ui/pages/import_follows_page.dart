...
class _ImportFollowsPageState extends ConsumerState<ImportFollowsPage> {
  ...
  void _onSearchChanged() {
    final query = _searchController.text.toLowerCase();
    setState(() {
      _filteredFollows = _allActors.where((actor) => 
          actor.handle.toLowerCase().contains(query) || 
          (actor.displayName?.toLowerCase().contains(query) ?? false)
        ).toList();
    });
      ...
      error: (error, stackTrace) => Scaffold(
        ...
        body: Center(
          child: Column(
            ...
            children: [
              Text('Failed to load follows: ${error.toString()}', 
                            ...
                            return ListTile(
                              leading: CircleAvatar(
                                backgroundImage: follow.avatar != null 
                                    ? CachedNetworkImageProvider(follow.avatar!) 
                                    : null,
                                child: follow.avatar == null 
